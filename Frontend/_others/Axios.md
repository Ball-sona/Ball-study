...
- Axis interceptor 란 요청을 보내거나 응답이 오기 직전에 호출되는 함수를 의미한다. 
- 이를 이용해서 요청이나 응답을 수정하고 컨트롤할 수 있다. 보통 헤더를 추가하거나, 요청을 수정하거나, 에러 핸들링 등에 사용된다. 

### Intercepting Request

Interceptor를 사용하여 서버에 요청을 보내기 직전에 요청 내용을 수정할 수 있다. 예를 들어 요청 직전에 현재 시각을 메타 데이터에 추가하여 요청을 보내면, 응답을 받기까지 얼마나 시간이 걸리는지 계산해볼 수 있다.
...
Interceptor을 통해 에러 로깅을 하거나, 데이터를 파싱하거나, 실패한 요청을 재요청할 수 있다. 아래 예시는 요청이 얼마나 걸렸는지 계산해주는 코드이다. 
