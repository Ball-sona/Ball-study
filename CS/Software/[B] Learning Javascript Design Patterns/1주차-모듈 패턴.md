...
- 모듈 패턴이란, 코드의 재사용성과 유지보수성을 높이기 위해 '캡슐화'와 '정보 은닉'을 실현하는 것
- 전역 스코프 오염을 방지하고, 공개 API만 노출하며 나머지 구현 세부사항은 비공개로 유지하는 것이 핵심

## 클로저

- 함수가 자신이 생성될 때의 렉시컬 환경을 기억하여, 함수가 실행된 이후에도 그 환경에 접근할 수 있도록 하는 매커니즘
- 이를 활용하여 외부에서는 접근할 수 없는 비공개 변수 생성 가능
...
const person = new Person('sona', 26);
person.getName(); // sona
```

- 각 인스턴스마다 별도의 클로저, 즉 별도의 비공개 변수가 생성된다. -> 메모리 사용량 증가
- 상속이나 프로토타입 기반 확장이 어렵다. -> 확장성 제약

## WeakMap

- 키가 약하게 참조되는 키-값 쌍의 컬렉션으로, 키는 반드시 객체여야 한다.
- 약한 참조 -> 키로 사용되는 객체에 대한 참조가 전부 사라지면, 해당 객체는 가비지 컬렉션 가능해짐.
...
class Person {
  ...
  getName() {
    privateMap.get(this).name;
  }
}

const person = Person('sona', 26);
person.getName(); // 26
```
