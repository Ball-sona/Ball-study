...
`+`나 `Number()` 처럼 문자열을 숫자형으로 변환하는 데 사용되지만, 변환할 수 없는 문자가 섞여있어도 에러를 발생하지 않고 그 앞 숫자까지만 변환한다는 점에서 차이가 있다.
...
`===` 처럼 값을 비교할 때 사용되는 메서드이나, 아래 두가지 케이스에서 `===` 와 다른 결과 보여준다.
